...
if [[ "${PHOENIX_LOG_AR_UP}" == 1 ]]; then
  ...
  /bin/bash "${PHOENIX_SCRIPTS}/ci-upload-artifacts-phoenix.sh" "${target_artifact}" > >("${PHOENIX_TEE}" -a "${AR_UP_LOG_FILE}") 2>&1 || exit 1
else
  /bin/bash "${PHOENIX_SCRIPTS}/ci-upload-artifacts-phoenix.sh" "${target_artifact}" || exit 1
fi
